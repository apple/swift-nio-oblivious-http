...
public struct BHTTPParser {
    ...
    while true {
      ...
      case .awaitingRequestHead(let knownLength):
        parseResult = try Self.parseRequestHead(
          &buffer, knownLength: knownLength, completeBodyReceived: self.readEOF)
      ...
      case .awaitingContent(let content):
        parseResult = try Self.parseContent(
          &buffer, content: content, completeBodyReceived: self.readEOF, role: self.role)
      case .awaitingTrailers(let knownLength):
        parseResult = try Self.parseTrailers(
          &buffer, knownLength: knownLength, completeBodyReceived: self.readEOF, role: self.role)
      case .complete:
  ...
  static func parseRequestHead(
    _ buffer: inout ByteBuffer, knownLength: Bool, completeBodyReceived: Bool
  ) throws -> ParseResult {
    ...
    return .complete(
      .request(.head(head)), nextState: .awaitingContent(.init(knownLength: knownLength)))
  }
  ...
  static func parseContent(
    _ buffer: inout ByteBuffer, content: State.Content, completeBodyReceived: Bool, role: Role
  ) throws -> ParseResult {
    switch content {
  ...
  static func parseTrailers(
    _ buffer: inout ByteBuffer, knownLength: Bool, completeBodyReceived: Bool, role: Role
  ) throws -> ParseResult {
  ...
  static func parseFieldSection(_ fieldSection: inout ByteBuffer, into headers: inout HTTPHeaders)
    throws
  {
